...
# Corrected path: the file is in the current directory, not in a 'data' subfolder
csv_path = Path("./sunspots_for_prophet.csv")
if not csv_path.exists():
    st.error("❌ sunspots_for_prophet.csv 파일을 찾을 수 없습니다. 루트 폴더에 있는지 확인하세요.")
    st.stop()
...
# 11년 주기(태양흑점 주기) 커스텀 seasonality 추가 (단위: days)
...
future = model.make_future_dataframe(periods=30, freq="YS")  # Year Start
...
fig2.suptitle("Forecast Components", fontsize=16)
st.pyplot(fig2)
